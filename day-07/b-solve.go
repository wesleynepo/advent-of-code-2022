...
func main() {
    ...
    amount := start.totalSize()
    total := 70000000 - amount
    fmt.Println(total)
    outputFolder(start, total, &amount)
    fmt.Println(amount)
}

func outputFolder(input *Folder, sum int, amount *int) {
    total := input.totalSize()

    if (total + sum > 30000000 && total < *amount) {
        *amount = total
    }

    for _, folder := range input.subfolders {
        outputFolder(folder, sum, amount)
    }
}
