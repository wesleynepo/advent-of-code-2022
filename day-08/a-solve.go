...
func main() {
    ...
    initial := 2 * rowLength + 2 * (columnLength - 2)

    calculate(grid, transpose(grid), &initial)
    fmt.Println(initial)
}
...
func calculate(matrix [][]int, transposed [][]int, sum *int) {
    ...
    for i := 1; i < columnLength - 1; i++ {
        for j := 1; j < rowLength - 1; j++ {
            ...
            if maxSlice(leftSlice) < actual || maxSlice(rightSlice) < actual || maxSlice(upSlice) < actual || maxSlice(downSlice) < actual{
                *sum++
            }
        }
    }
